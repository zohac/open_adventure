...
void main() {
  ...
  group('initialGame metadata fallback', () {
    test('falls back to first location when metadata has invalid keys', () async {
      final assets = _MockAssets();
      when(() => assets.getLocations()).thenAnswer((_) async => [
            {'name': 'LOC_START', 'description': {}},
            {'name': 'LOC_HILL', 'description': {}},
          ]);
      when(() => assets.loadMap(any())).thenAnswer((_) async => {'invalid': true});
